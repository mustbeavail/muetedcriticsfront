...
export default function WinningRate_Playtime_UserStats() {
    ...
    const winCount = filtered.filter(i => i.vitory_or_defeat === "victory").length;
    const totalCount = filtered.length;
    const winRate = totalCount === 0 ? 0 : (winCount / totalCount) * 100;

    const heroPlaytimeData = getHeroPlaytimeData(filtered);
    const mainRoleInfo = getMainRoleInfo(filtered);
    const modePlaytimeData = getModePlaytimeData(filtered);
        ...
        <div className={"userStats-chartWrapper"}>
            ...
            {filtered.map((item) => (
                <div className="userStats-result" key={item.result_idx}>
                    <h3>{item.user_id}</h3>
                    <p>승리 <span className="win">{filtered.filter(i => i.vitory_or_defeat === "victory").length} </span>
                        / 패배 <span className="lose">{filtered.filter(i => i.vitory_or_defeat === "defeat").length}</span></p>
